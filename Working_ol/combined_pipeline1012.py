...
def normalize_device(d):
    ...
    if s.startswith('cuda'):
        return s  # 'cuda' or 'cuda:0'
    if s.isdigit():
        return f'cuda:{s}'  # '0' -> 'cuda:0'
    return s
...
# ============= Simple Kalman filter for the ball =============
class KF2D:
    def __init__(self, x, y):
        self.x = np.array([[x, y, 0., 0.]], dtype=float).T
        self.P = np.eye(4) * 100.0
        self.Q = np.diag([4, 4, 10, 10])  # process noise
        self.R = np.diag([25, 25])        # measurement noise
        self.F = np.eye(4)                # will fill dt each step
...
def detect_stumps_and_crease(frame):
    ...
    # Vertical lines (far half of image)
    ...
    # Popping crease: bright horizontal near near-end
...
def detect_ball_in_roi(model_det, frame, cx, cy, box_px=160,
                       conf=0.10, imgsz=640, class_name="sports ball"):
  
    H, W = frame.shape[:2]
    x0 = max(0, int(cx - box_px // 2))
    y0 = max(0, int(cy - box_px // 2))
    x1 = min(W, int(cx + box_px // 2))
    y1 = min(H, int(cy + box_px // 2))

    if x1 - x0 < 10 or y1 - y0 < 10:
        return None, None

    crop = frame[y0:y1, x0:x1].copy()
    res = model_det.predict(crop, imgsz=imgsz, conf=conf, verbose=False)[0]
    ...
    cx_full = x0 + (bx0 + bx1) / 2.0
    cy_full = y0 + (by0 + by1) / 2.0

    return (cx_full, cy_full), best_conf
...
def main():
    ...
    ap.add_argument("--scale_m_per_px", type=float, default=None,
                    help="Manual scale (meters per pixel). If not given, auto-compute from stump height")
    ...
    ap.add_argument("--handed", choices=["right", "left"], default="right",
                    help="Bowling arm; assumes ball in this hand until release")
    ...
    ap.add_argument("--bowler_height_m", type=float, default=None,
                    help="Bowler height in meters (for scale if stumps not detected)")
    ap.add_argument("--gate_ball_from", choices=["none", "ffc"], default="ffc",
                help="If 'ffc', ignore ball detection until front-foot contact.")

    args = ap.parse_args()

    kpts_all = []   # <--- add this
    frame_ids = []  # safe to initialize here too
    # ------------------ Video I/O ------------------
    ...
    # Scale: meters per pixel (auto from stump height if not provided)
    m_per_px = args.scale_m_per_px
    if m_per_px is None:
        if st_y0 is not None and st_y1 is not None:
            stump_h_px = max(1, st_y1 - st_y0)
            m_per_px = STUMP_HEIGHT_M / stump_h_px
            print("[INFO] Scale from stumps:", m_per_px)
        elif args.bowler_height_m is not None:
            # Use bowler height as fallback
            res_p = m_pose.predict(first, imgsz=args.imgsz_pose, verbose=False)[0]
            k0, _ = pick_person_keypoints(res_p)
            if k0 is not None:
                top = k0[COCO_KPTS["nose"]][1]
                bottom = max(k0[COCO_KPTS["lank"]][1], k0[COCO_KPTS["rank"]][1])
                pixel_height = abs(bottom - top)
                if pixel_height > 0:
                    m_per_px = args.bowler_height_m / pixel_height
                    print("[INFO] Scale from bowler height:", m_per_px)

    if m_per_px is None:
        m_per_px = 0.0045  # sensible default (approx 1px ≈ 4.5mm at broadcast zoom)
        print("[WARN] Using default scale m/px =", m_per_px)
    ...
    while True:
        ...
        if detections:
            if pred is not None:
                # Score by confidence and distance from prediction
                gating_radius = 80.0
                scored = []
                ...
                # First detection → take highest confidence
        ...
        # Final choice: detection if available, else KF prediction
        ...
        if release_idx is None and kpts is not None and p is not None:
            rw = kpts[COCO_KPTS["rwri"]]
            lw = kpts[COCO_KPTS["lwri"]]
            d_r = float(np.hypot(p[0] - rw[0], p[1] - rw[1]))
            d_l = float(np.hypot(p[0] - lw[0], p[1] - lw[1]))
            dmin = min(d_r, d_l)
            ...
            # persistent counters (attach to function scope once)
            if not hasattr(main, "_near_cnt"):
                main._near_cnt = 0
                main._far_cnt = 0
                main._last_near_idx = None

            cur_idx = len(ball_track)

            if dmin < HAND_NEAR:
                main._near_cnt += 1
                main._far_cnt = 0
                main._last_near_idx = cur_idx - 1
            elif dmin > HAND_FAR:
                if main._near_cnt >= NEAR_MIN:
                    main._far_cnt += 1
                    if main._far_cnt >= FAR_MIN:
                        candidate = (main._last_near_idx or (cur_idx - 2)) + 1
                        candidate = max(0, min(candidate, cur_idx - 1))
                        release_idx = candidate
                        release_time = ball_track[release_idx]["t"] if candidate < len(ball_track) else t
                else:
                    main._near_cnt = 0
                    main._far_cnt = 0
            # else in-between: do nothing
        ...
        put_multiline_text(overlay,
                           [f"FPS:{fps:.2f}",
                            f"Scale m/px:{m_per_px:.5f}" if m_per_px else "Scale:auto?"],
                           (10, 30))
    ...
    if release_idx is not None and len(ball_track) > release_idx:
    ...
    # ------------------ Compute bowler events only (no speeds here) ------------------
    bfc_idx, ffc_idx, info = detect_bfc_ffc(kpts_all, fps=fps)
    ...
    print(f"[DONE] ball track csv: {args.out_csv_ball}")
